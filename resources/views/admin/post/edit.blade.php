                        ...
                        <div class="card-body">
                            ...
                            <div class="tab-content mt-3" id="custom-tabs-five-tabContent">
                                <div class="tab-pane fade show active" id="RU" role="tabpanel" >
                                    <div class="form-group ">
                                        ...
                                        <input name="name_ru" type="text" class="form-control  @error('name_ru') is-invalid @enderror" placeholder="Название" value="{{$post->name_ru}}">
                                        @error('name_ru')<p class="text-danger">{{ $message }}</p> @enderror
                                    </div>

                                    <div class="form-group">
                                        <label>Описание <sup>RU</sup></label>
                                        <textarea  name="content_ru" class="summernote form-control @error('content_ru') is-invalid @enderror" rows="13" placeholder="Описание">{{$post->content_ru}}</textarea>
                                ...
                                <div class="tab-pane fade" id="RO" role="tabpanel" >
                                    <div class="form-group">
                                        ...
                                        <input name="name_ro" type="text" class="form-control  @error('name_ro') is-invalid @enderror" placeholder="Название" value="{{$post->name_ro}}">
                                        @error('name_ro')<p class="text-danger">{{ $message }}</p> @enderror
                                    </div>

                                    <div class="form-group ">
                                        <label>Описание<sup>RO</sup></label>
                                        <textarea  name="content_ro" class="summernote form-control @error('content_ro') is-invalid @enderror" rows="13" placeholder="Описание">{{ $post->content_ro }}</textarea>
                                ...
                                <div class="tab-pane fade" id="EN" role="tabpanel">
                                    <div class="form-group ">
                                        ...
                                        <input name="name_en" type="text" class="form-control  @error('name_en') is-invalid @enderror" placeholder="Название" value="{{$post->name_en}}">
                                        @error('name_en')<p class="text-danger">{{ $message }}</p> @enderror
                                    </div>

                                    <div class="form-group">
                                        <label>Описание<sup>EN</sup></label>
                                        <textarea name="content_en" class="summernote form-control @error('content_en') is-invalid @enderror" rows="13" placeholder="Описание">{{ $post->content_en }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <select name="categories_id[]" class="select2 @error('categories_id') is-invalid @enderror" multiple="multiple" data-placeholder="Select a State" style="width: 100%;">
                                    @foreach($categories as $category)
                                            <option @if($post->categories->contains('id', $category->id)) selected @endif value="{{$category->id}}">{{$category->name}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mt-5">
                                ...
                                <div id="files-names" class="row">
                                    @if($post->preview_picture !== null)
                                        <div class="col-6 position-relative p-1">
                                            <i data-id="0" class="file-delete nav-icon fas fa-trash text-danger position-absolute" style="top:20px; left:20px;cursor:pointer"></i>
                                            <input type="hidden"name="current_image" value="{{$post->preview_picture}}">
                                            <img width="100%" src="{{$post->preview_picture}}"/>
                                        </div>
                                    @endif
